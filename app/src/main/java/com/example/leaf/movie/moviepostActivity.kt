...
import com.example.leaf.databinding.ActivityMoviepostBinding
...
class moviepostActivity : AppCompatActivity() {
    private lateinit var binding : ActivityMoviepostBinding
    private lateinit var key: String
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = DataBindingUtil.setContentView(this,R.layout.activity_moviepost)
    ...
    private fun showDialog(){
        ...
        alertDialog.findViewById<Button>(R.id.deletebtn)?.setOnClickListener{
            FBRef.movieRef.child(key).removeValue()
            finish()
        }
    }
        ...
        CoroutineScope(Dispatchers.Main).launch {
            storageReference.downloadUrl.addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Glide.with(this@moviepostActivity)
                        .load(task.result)
    ...
    private fun getBoardData(key: String){
        ...
        FBRef.movieRef.child(key).addValueEventListener(postListener)
    }
