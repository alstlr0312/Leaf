...
import com.example.leaf.databinding.ActivityHousepostBinding
...
class housepostActivity : AppCompatActivity() {
    private lateinit var binding : ActivityHousepostBinding
    private lateinit var key: String
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = DataBindingUtil.setContentView(this,R.layout.activity_housepost)
        ...
        CoroutineScope(Dispatchers.Main).launch {
            storageReference.downloadUrl.addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Glide.with(this@housepostActivity)
                        .load(task.result)
    ...
    private fun getBoardData(key: String){
        ...
        FBRef.houseRef.child(key).addValueEventListener(postListener)
    }
